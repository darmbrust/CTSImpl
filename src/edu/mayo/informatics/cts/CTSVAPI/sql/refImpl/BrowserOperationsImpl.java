...
import edu.mayo.informatics.cts.CTSVAPI.lucene.LuceneSearch;
...
public class BrowserOperationsImpl implements BrowserOperations
{
    ...
    public final static Logger    logger                    = Logger.getLogger("edu.mayo.informatics.cts.VAPI_sql_Browser");

    /**
     * Create a new Browser Operations - using current values of CTSConstants.
     * @return
     * @throws UnexpectedError
     */
    static public synchronized BrowserOperations _interface() throws UnexpectedError
    {
        BrowserOperationsImpl svc = new BrowserOperationsImpl(null, null, null, null, null, false, false);
    ...
    {
        BrowserOperationsImpl svc = new BrowserOperationsImpl(userName, userPassword, server, driver, tablePrefix, loadProps, initLogger);
        return (BrowserOperations) svc;
    }
...

    {
        ...
        try
        {
            ...
            svc = new BrowserOperationsImpl(username, password, temp.server, temp.driver, temp.tablePrefix, loadProps, initLogger);
        }
    ...
    public BrowserOperationsImpl()
    {
        try
        {
            ...
            roi_ = new RuntimeOperationsImpl(null, null, null, null, null, false, false);
        }
        catch (Exception e)
        {
    ...
    protected BrowserOperationsImpl(String username, String password, String server, String driver, String tablePrefix, boolean loadProps, boolean initLogger)
            throws UnexpectedError
    {
        try
        {
            ...
            roi_ = new RuntimeOperationsImpl(username, password, server, driver, tablePrefix, false, false);
        }
        catch (Exception e)
        {
            logger.error("BrowserOperationsImpl Constructor error", e);
...
//                }
            ...
            UnexpectedError, TimeoutError
    {
        ...
        try
        {
            ...
            String[] supportedAssociations = roi_.getSupportedAssociations(codeSystemName);
            ...
            getAssociationProperties.setString(2, roi_.getNativeRelation(codeSystemName));
        ...
        try
        {
            ...
            if ((codeToExpand.getConcept_code() == null) || (codeToExpand.getConcept_code().length() == 0))
            {
                ...
                else
                {
                    ...
                    //create a new one, so it doesn't mess up the input conceptId.
            ...
            long startTime) throws TimeoutError, UnexpectedError
    {
        ...
        try
        {
            ...
            getChildrenOfConcept.setString(1, conceptCode);
            getChildrenOfConcept.setString(2, codingSchemeName);
            getChildrenOfConcept.setString(3, roi_.getNativeRelation(codingSchemeName));
            getChildrenOfConcept.setString(4, relationship);
            ...
            while (queryResults.next())
            {
                ...
                String currentTargetCodingSchemeName = queryResults.getString("targetCodingSchemeName");
                ...
                conceptId.setCodeSystem_id(roi_.getIdForRelationshipCodeSystemName(codingSchemeName,
                                                                                   currentTargetCodingSchemeName));
                try
                {
                ...
                relatedCode.setRelationQualifiers(roi_.getRelationQualifiers(codingSchemeName, queryResults
                ...
                if (isTransitive)
                {
                    checkForChildren.setString(1, relatedCode.getConcept_code());
                    checkForChildren.setString(2, codingSchemeName);
                    checkForChildren.setString(3, roi_.getNativeRelation(codingSchemeName));
                    checkForChildren.setString(4, relationship);
            ...
            long startTime) throws TimeoutError, UnexpectedError
    {
        ...
        try
        {
            ...
            getParentsOfConcept.setString(1, conceptCode);
            getParentsOfConcept.setString(2, codingSchemeName);
            getParentsOfConcept.setString(3, roi_.getNativeRelation(codingSchemeName));
            getParentsOfConcept.setString(4, relationship);
            ...
            while (queryResults.next())
            {
                ...
                String currentSourceCodingSchemeName = queryResults.getString("sourceCodingSchemeName");
                ...
                if (isTransitive)
                {
                    checkForParents.setString(1, relatedCode.getConcept_code());
                    checkForParents.setString(2, codingSchemeName);
                    checkForParents.setString(3, roi_.getNativeRelation(codingSchemeName));
                    checkForParents.setString(4, relationship);
            ...
            throws TimeoutError, UnexpectedError
    {
        ...
        try
        {
            ...
            while (results.next())
            {
                ...
                relatedCode.setDesignation(results.getString("entityDescription"));
            ...
            UnknownCodeSystem, UnexpectedError
    {
        ...
        try
        {
            ...
            getCodeDetails.setString(1, concept_id.getConcept_code());
            getCodeDetails.setString(2, codeSystemName);
            ...
            String nativeRelation = "";
            try
            {
                nativeRelation = roi_.getNativeRelation(codeSystemName);
            }
            catch (UnexpectedError e)
            {
                return result;
                // do nothing - this means that there is no nativeRelation - hence no relationships
            }

            ArrayList sourceFor = new ArrayList();

            getSourceFor = queries_.checkOutStatement(queries_.GET_SOURCE_FOR_ALL);
            getSourceFor.setString(1, concept_id.getConcept_code());
            getSourceFor.setString(2, codeSystemName);
            getSourceFor.setString(3, nativeRelation);
            ...
            while (queryResults.next())
            {
                ...
                // source code system can be different than that code system we are in.
                ConceptId tempSourceConceptId = new ConceptId();
                tempSourceConceptId.setCodeSystem_id(roi_
                        .getIdForRelationshipCodeSystemName(codeSystemName, queryResults
                                .getString("sourceCodingSchemeName")));
                tempSourceConceptId.setConcept_code(concept_id.getConcept_code());
                temp.setSourceConcept_id(tempSourceConceptId);
                ...
                temp.setRelationQualifiers(roi_.getRelationQualifiers(codeSystemName, queryResults
                        .getString("multiAttributesKey")));
            ...
            result.setSourceFor((ConceptRelationship[]) sourceFor.toArray(new ConceptRelationship[sourceFor.size()]));

            // get the target of
            ArrayList targetOf = new ArrayList();

            getTargetOf = queries_.checkOutStatement(queries_.GET_TARGET_OF_ALL);
            getTargetOf.setString(1, concept_id.getConcept_code());
            getTargetOf.setString(2, codeSystemName);
            getTargetOf.setString(3, nativeRelation);
            ...
            while (queryResults.next())
            {
                ...
                ConceptRelationship temp = new ConceptRelationship();
                temp.setRelationship_code(queryResults.getString("association"));
                temp.setTargetConcept_id(concept_id);
                // source code system can be different than that code system we are in.
                ConceptId tempTargetConceptId = new ConceptId();
                tempTargetConceptId.setCodeSystem_id(roi_
                        .getIdForRelationshipCodeSystemName(codeSystemName, queryResults
                                .getString("targetCodingSchemeName")));
            ...
            result.setTargetOf((ConceptRelationship[]) targetOf.toArray(new ConceptRelationship[targetOf.size()]));
            ...
            UnknownMatchAlgorithm
    {
        ...
        try
        {
            ...
            {
                return getLuceneSearch().luceneLookupConceptCodesByDesignation(codeSystemName,
                                                                                                  matchText,
                                                                                                  matchAlgorithm_code,
                                                                                                  language_code,
                                                                                                  activeConceptsOnly,
            ...
            query.append("SELECT " + queries_.getTableName(SQLTableConstants.CONCEPT_PROPERTY) + ".conceptCode");
            query.append(" FROM " + queries_.getTableName(SQLTableConstants.CONCEPT_PROPERTY));
            if (activeConceptsOnly)
            {
                query.append(" INNER JOIN " + queries_.getTableName(SQLTableConstants.CONCEPT));
                query.append(" ON " + queries_.getTableName(SQLTableConstants.CONCEPT_PROPERTY) + ".codingSchemeName = " + queries_.getTableName(SQLTableConstants.CONCEPT) + ".codingSchemeName");
                query.append(" AND " + queries_.getTableName(SQLTableConstants.CONCEPT_PROPERTY) + ".conceptCode = " + queries_.getTableName(SQLTableConstants.CONCEPT) + ".conceptCode");

            }
            query.append(" WHERE " + queries_.getTableName(SQLTableConstants.CONCEPT_PROPERTY) + ".codingSchemeName=?");
            
            if (queries_.supports2006Model())
            {
                query.append(" AND " + queries_.getTableName(SQLTableConstants.CONCEPT_PROPERTY) + ".propertyType='presentation'");
            }
            else
            {
                query.append(" AND " + queries_.getTableName(SQLTableConstants.CONCEPT_PROPERTY) + ".property='textualPresentation'");
            }
            if (matchText != null && matchText.length() > 0)
            {
                query.append(" AND " + queries_.getTableName(SQLTableConstants.CONCEPT_PROPERTY) + ".propertyValue {LIKE} ?");
            }
            ...
            if (language_code != null && language_code.length() > 0)
            {
                if (language_code.equals(roi_.getDefaultLanguage(codeSystemName)))
                {
                    ...
                    query
                            .append(" AND (" + queries_.getTableName(SQLTableConstants.CONCEPT_PROPERTY) + ".language {LIKE} ? OR " + queries_.getTableName(SQLTableConstants.CONCEPT_PROPERTY) + ".language is NULL OR " + queries_.getTableName(SQLTableConstants.CONCEPT_PROPERTY) + ".language='')");
                }
                else
                {
                    query.append(" AND " + queries_.getTableName(SQLTableConstants.CONCEPT_PROPERTY) + ".language {LIKE} ?");
                }
            }
            if (activeConceptsOnly)
            {
                query.append(" AND " + queries_.getTableName(SQLTableConstants.CONCEPT) + ".isActive <> {false}");
            }
            // TODO - find out if it is quicker to check for the active status after the query, rather than joining.
            ...
            getCodesByDesignation.setMaxRows(sizeLimit * 5); // make it bigger, because it will usually match on
                                                                // multiple
            // designations per concept, and I will end up returning less concepts than the limit requested in that
            // case.
            ...
            if (language_code != null && language_code.length() > 0)
            {
                ...
                if (queries_.requiresUppercasing())
                {
            ...
            UnknownPropertyCode, UnexpectedError, UnknownMatchAlgorithm, TimeoutError
    {
        ...
        try
        {
            ...
            if (matchAlgorithm_code != null && matchAlgorithm_code.endsWith("LuceneQuery"))
            {
                return getLuceneSearch().luceneLookupConceptCodesByProperty(codeSystemName,
                                                                                               matchText,
                                                                                               matchAlgorithm_code,
                                                                                               language_code,
                                                                                               activeConceptsOnly,
            ...
            query.append("SELECT " + queries_.getTableName(SQLTableConstants.CONCEPT_PROPERTY) + ".conceptCode FROM " + queries_.getTableName(SQLTableConstants.CONCEPT_PROPERTY));
            if (activeConceptsOnly)
            {
                query
                        .append(" INNER JOIN " + queries_.getTableName(SQLTableConstants.CONCEPT) + " ON " + queries_.getTableName(SQLTableConstants.CONCEPT_PROPERTY) + ".codingSchemeName = " + queries_.getTableName(SQLTableConstants.CONCEPT) + ".codingSchemeName AND " + queries_.getTableName(SQLTableConstants.CONCEPT_PROPERTY) + ".conceptCode = " + queries_.getTableName(SQLTableConstants.CONCEPT) + ".conceptCode");
            }
            query.append(" WHERE " + queries_.getTableName(SQLTableConstants.CONCEPT_PROPERTY) + ".codingSchemeName = ?");
            if (matchText != null && matchText.length() > 0)
            {
            ...
            if (mimeTypes != null && mimeTypes.length != 0)
            {
                ...
                for (int i = 0; i < mimeTypes.length; i++)
                {
                    query.append("" + queries_.getTableName(SQLTableConstants.CONCEPT_PROPERTY) + ".presentationFormat=?");
            ...
            if (properties != null && properties.length != 0)
            {
                ...
                for (int i = 0; i < properties.length; i++)
                {
                    query.append(queries_.getTableName(SQLTableConstants.CONCEPT_PROPERTY) + ".property=?");
            ...
            UnknownLanguageCode, UnexpectedError, UnknownMatchAlgorithm
    {
        ...
        try
        {
            ...
            if (matchAlgorithm_code != null && matchAlgorithm_code.endsWith("LuceneQuery"))
            {
                ConceptDesignation[] temp = getLuceneSearch().luceneLookupDesignations(codeSystemName, conceptCode.getConcept_code(), matchText,
            ...
            StringBuffer query = new StringBuffer(
                    "SELECT language, propertyValue, isPreferred"
                            + " FROM " + queries_.getTableName(SQLTableConstants.CONCEPT_PROPERTY)
                            + " WHERE conceptCode=?"
                            + " AND codingSchemeName=?"
                            + " AND " + (queries_.supports2006Model() ? "propertyType='presentation'" : "property='textualPresentation'"));
            ...
            if (languageCode != null && languageCode.length() != 0)
            {
                ...
                {
                    query
                            .append(" AND (language = ? OR language is null OR language='')");
                }
                else
                {
            ...
            if (matchText != null && matchText.length() > 0)
            {
                query.append(" AND propertyValue {LIKE} ?");
            }
            ...
            //TODO - language handling in this method does not match the spec. should try for a more general
            //match of no results are found.  Broke in ldap and sql Lite too.
            while (results.next())
            {
                ...
                String propertyText = results.getString("propertyValue");
            ...
            UnexpectedError, UnknownMatchAlgorithm
    {
        ...
        try
        {
            ...
            if (matchAlgorithm_code != null && matchAlgorithm_code.endsWith("LuceneQuery"))
            {
                ConceptProperty[] temp = getLuceneSearch().luceneLookupProperties(codeSystemName, concept_id.getConcept_code(), properties, matchText,
            ...
            StringBuffer query = new StringBuffer();
            query
                    .append("SELECT property, propertyValue, language, presentationFormat");
            query.append(" FROM " + queries_.getTableName(SQLTableConstants.CONCEPT_PROPERTY));
            query.append(" WHERE conceptCode=?");
            query.append(" AND codingSchemeName=?");
            ...
            if (mimeTypes != null && mimeTypes.length > 0)
            {
                ...
                for (int i = 0; i < mimeTypes.length; i++)
                {
                    query.append(" presentationFormat=?");
                    if (i + 1 < mimeTypes.length)
                    {
                        query.append(" OR");
                    }
            ...
            while (results.next())
            {
                ...
                conceptProperty.setMimeType_code(results.getString("presentationFormat"));
                conceptProperty.setProperty_code(results.getString("property"));
                conceptProperty.setPropertyValue(results.getString("propertyValue"));
    ...
    public String getServiceName() throws java.rmi.RemoteException, UnexpectedError
    {
        ...
        try
        {
            return "Mayo CTS Vapi SQL Browser";
        }
        catch (Exception e)
        {
            logger.error("Unexpected Error", e);;
            ...
            throws UnexpectedError
    {
        ...
        try
        {
            ...
            //validate attributes - return true
            if (CTSConstants.EVSModeEnabled.getValue() && codingScheme.equals("*"))
            {
                return true;
            }
