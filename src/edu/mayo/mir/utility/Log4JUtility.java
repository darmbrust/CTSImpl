...
public class Log4JUtility
{
     ...
     * 
     * If the log4j file path specified in the properties object is a relative (or short) path, this method will prepend
     * the value of 'propertiesFileParentFolder' to the path first.
     * 
     * The method PropertiesUtility.loadPropertiesFromFileOrURL will assign the necessary information automatically.
     * 
     * It is recommended that the Properties object that you pass into this class be created with the
     * PropertiesUtility.loadPropertiesFromFileOrURL method.
     * 
     * @param properties The properties object to look in for the file name of the log4j config file.
     * @param log4JConfigFilePropertyName The property name in the properties object that has a corresponding value with
     *            the path of the log4j config file.
     * @param setLogOutputBaseDirToPropertiesPath Should the log4j config file be modifed so that all appenders that end
     *            with "File" have the base directory of the location of the properties file prepended to them? If true -
     *            this also requires having the 'propertiesFileParentFolder' value set.
     * 
     * @throws Exception
     */
            ...
            String log4JConfigFilePropertyName, boolean setLogOutputBaseDirToPropertiesPath) throws Exception
    {
        ...
        {
            throw new Exception("No value specified for '"
                    + log4JConfigFilePropertyName
    ...
    /**
     * Method to read in a log4j config file, modify every property that ends with "File" such that the baseFilePath
     ...
     * 
     * @param log4JConfigFile The file (or URL) to read the log4j configuration from
     * @param baseLogOutputDirectory The path to prefix onto all values that end with "File" in the log4j config file.
     * @throws IOException
     */
     ...
     * 
     * @param log4JConfigFile File (or URL) to use to configure log4j.
     * @throws IOException
     */
     ...
     * 
     * @param props The log4j properties
     */
     ...
     * Configure log4j from a properties object.
     * 
     * Modifies every property that ends with "File" such that the baseFilePath value is prefixed onto it, and then
     * configure log4j with the results.
     * 
     * Useful for making log4j configuration work with relative file paths.
     * 
     * @param props properties to load.
     * @param baseLogOutputDirectory directory path to prefix onto items ending with "File"
     */
