...
/**
 * This class is an automatically reconnecting ldap dir context.
 * If the connection times out, this will automatically reconnect it.
 * This class should simply extend InitialDirContext, but it is illegal to do
 * this = super(); so I have to implement it like this... yuk.  It needs to be redone to implement the 
 * ldap api, but that hasn't happened yet.
 * 
 * @author <A HREF="mailto:armbrust.daniel@mayo.edu">Dan Armbrust</A>
 */
public class AutoReconnectDirContext
{
	//TODO - this should implement the ldap interface...
    public int MAX_RECONNECT_ATTEMPTS = 5;

    private DirContext context_;
    public final static org.apache.log4j.Logger logger = Logger.getLogger("edu.mayo.informatics.cts.utility.AutoReconnectDirContext");
    ...
    public AutoReconnectDirContext(Hashtable env, boolean logPassword) throws javax.naming.NamingException
    {
        ...
        if (logPassword)
            logger.debug(HashTableUtility.hashTableToString(env));
        else
            logger.debug(HashTableUtility.hashTableToString(env, new String[] { "java.naming.security.credentials" }));
    ...
    public void rename(Name oldName, Name newName) throws javax.naming.NamingException
    {
        logger.debug(
            "rename: oldName: "
                + (oldName == null ? "null" : oldName.toString())
                + " newName: "
    ...
    {
        logger.debug("composeName: name: " + (name == null ? "null" : name) + " prefix: " + (prefix == null ? "null" : prefix));
        try
        {
    ...
    {
        logger.debug("search: name: " + (name == null ? "null" : name.toString()) + " filter: " + (filter == null ? "null" : filter));
        try
        {
    ...
    {
        logger.debug("rename: oldName: " + (oldName == null ? "null" : oldName) + " newName: " + (newName == null ? "null" : newName));
        try
        {
            context_.rename(oldName, newName);
        }
    ...
    {
        logger.debug("addToEnviroment: propName: " + (propName == null ? "null" : propName) + " propVal: " + propVal.toString());
        try
        {
    ...
    {
        logger.debug(
            "getAttributes: name: " + (name == null ? "null" : name) + " AttributeIds: " + StringArrayUtility.stringArrayToString(attrIds));
        try
        {
    ...
    {
        logger.debug("search: name: " + (name == null ? "null" : name) + " filter: " + (filter == null ? "null" : filter));
        try
        {
            return context_.search(name, filter, cons);
        }
    ...
    public NamingEnumeration search(String name, Attributes attrs, String[] attrsToReturn) throws javax.naming.NamingException
    {
        logger.debug(
            "search: name: "
                + (name == null ? "null" : name)
                + " AttributesToReturn: "
                + StringArrayUtility.stringArrayToString(attrsToReturn));
        try
        {
    ...
    {
        logger.debug(
            "search: name: " + (name == null ? "null" : name.toString()) + " filterExpr: " + (filterExpr == null ? "null" : filterExpr));
        try
        {
        ...
        throws javax.naming.NamingException
    {
        logger.debug(
            "search: name: "
                + (name == null ? "null" : name.toString())
                + " AttributesToReturn: "
                + StringArrayUtility.stringArrayToString(attributesToReturn));
        try
        {
    ...
    public Attributes getAttributes(Name name, String[] attrIds) throws javax.naming.NamingException
    {
        logger.debug(
            "getAttributes: name: "
                + (name == null ? "null" : name.toString())
                + " attrIds: "
                + StringArrayUtility.stringArrayToString(attrIds));
        try
        {
            return context_.getAttributes(name, attrIds);
        }
    ...
    public NamingEnumeration search(String name, String filterExpr, Object[] filterArgs, SearchControls cons)
        throws javax.naming.NamingException
    {
        logger.debug("search: name: " + (name == null ? "null" : name) + " filterExpr: " + (filterExpr == null ? "null" : filterExpr));
        try
        {
            return context_.search(name, filterExpr, filterArgs, cons);
        }
    ...
    private void reconnect() throws javax.naming.NamingException
    {
        ...
        while (true)
        {
            try
            {
                Hashtable env = context_.getEnvironment();
                context_.close();
                context_ = new InitialDirContext(env); //no exception means it succeeded
                break;
            }
            catch (NamingException namingException)
            {
                //When this happens, it just means that it couldn't reconnect... try again, until MAX_RECONNECT_ATTEMPTS is reached
                logger.warn(
                    "Network connection down, trying to reestablish.... "
                        + (MAX_RECONNECT_ATTEMPTS - 1 - reconnectAttempts)
                        + " Tries remaining");
                reconnectAttempts++;
